...
const EscolhaFavoritos = () => {
    ...
    return (
        <div className="flex items-center justify-center bg-white/20 p-2">
            <div className="w-full max-w-7xl bg-white/10 rounded-xl shadow-lg overflow-hidden transition-all">
                ...
                {/* Container principal */}
                <div className="flex flex-col w-full max-w-full md:w-[90%] xl:w-[75%] mx-auto m-4 border-2 border-white bg-gray-700/40 rounded-xl p-2 gap-4">

                    {/* Lista de categorias */}
                    <div className="border-b w-full border-white p-2 overflow-x-auto">
                        <div className="flex flex-wrap justify-center gap-4 min-w-fit">
                    ...
                    {/* Conteúdo principal */}
                    <div className="flex flex-col md:flex-row justify-center gap-6 h-auto md:h-[auto]">

                        {/* Lista de produtos */}
                        <div className={`w-full md:max-w-xs ${produtoSelecionado || produtoHover ? 'hidden md:block' : 'block'}`}>
                            {produtos.map((prod) => (
                                <div
                                    key={prod.nome}
                                    onClick={() => setProdutoSelecionado(prod)}
                                    onMouseEnter={() => setProdutoHover(prod)}
                                    className={`p-2 bg-transparent mt-4 rounded-lg shadow-sm hover:shadow-md cursor-pointer transition-all hover:bg-gray-900/30 border border-white ${produtoHover?.nome === prod.nome ? 'bg-gray-900/30' : ''
                                        }`}
                                >
                                    <div className="flex items-center justify-between">
                                        <h3 className="font-semibold text-white text-base sm:text-lg">{prod.nome}</h3>
                                        <button className="bg-purple-400/50 text-white hover:bg-purple-400 border-b-4 border-r-4 border-gray-900 px-3 sm:px-5 rounded-xl font-semibold transition-all ml-2 text-sm sm:text-base">
                        ...
                        {/* Detalhes do produto */}
                        <div className={`w-full md:w-1/2 flex items-center justify-center p-2 ${produtoSelecionado || produtoHover ? 'block' : 'hidden'} md:block`}>
                            {(produtoSelecionado || produtoHover) ? (
                                <div className="w-full h-[42vh] flex flex-col justify-between">
                                    ...
                                    <div className="flex flex-col items-center mb-8">
                                        <div className="flex justify-center w-full sm:w-[80%]">
                                            <img
                                                ...
                                                className="w-full h-48 object-cover border-2 border-white rounded-xl shadow-md"
                                            />
                                        </div>
